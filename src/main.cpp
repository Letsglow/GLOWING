...
#if defined(NDEBUG)
# error "Glow cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf6ded3b57068fd31003a3b183ae00e058e80914ade5fd1da129c62e02fd1947b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Glow: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Glow Signed Message:\n";
...
{
    // Glow: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Glow: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Glow: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1521075632, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 14 Mar 2018 Stephen Hawking Visionary physicist dies aged 76";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0433ced4c1fb877d082b1063f787a331e2662181f97d3ce0f4b61878cdaeb4b4fdb72ae842ae4c00b283442764c46c13cd8ded4eb3f53778251d8a668d98bb029c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521075632;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1764154;
        ...
        assert(block.hashMerkleRoot == uint256("0x3f04be7cfb6eb304d5d8bceeca7b363ac3dbecdbf1d50962b83e2747807fd81f"));
...
unsigned char pchMessageStart[4] = { 0x66, 0x3c, 0x77, 0xc9 }; // Glow: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlowMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlowMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlowMiner(CWallet *pwallet)
{
    printf("GlowMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("glow-miner");
    ...
    try { loop {
        ...
        printf("Running GlowMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlowMiner, pwallet));
}
